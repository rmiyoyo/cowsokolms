<x-app-layout>
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <h2 class="text-3xl font-bold text-gray-900 mb-8">Edit Quiz</h2>
          
            <form method="POST" action="{{ route('admin.quizzes.update', $quiz) }}" class="lms-card p-8">
                @csrf
                @method('PUT')
                    ...
                    @foreach($quiz->questions as $index => $question)
                        <div class="question-item border border-gray-200 p-6 rounded-lg mb-6">
                            <div class="mb-4">
                                ...
                                <textarea name="questions[{{ $index }}][question]" required class="lms-form-input" rows="2">{{ $question->question }}</textarea>
                            </div>
                            <div class="mb-4">
                            ...
                            <div class="mb-4 mcq-options">
                                ...
                                <textarea name="questions[{{ $index }}][options]" required class="lms-form-input" rows="3">{{ json_encode($question->options) }}</textarea>
                            </div>
                            <div class="mb-4">
                ...
                <button type="submit" class="lms-btn-primary">Update Quiz</button>
            </form>
        </div>
    </div>
    <script>
